...
@section('page-subtitle', 'Perbarui informasi akun supervisor')
...
<div class="max-w-xl mx-auto">
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
        ...
        <div class="bg-linear-to-r from-amber-500 to-orange-500 px-6 py-5">
            <h2 class="text-lg font-bold text-white">Edit Supervisor</h2>
            <p class="text-amber-100 text-sm mt-1">{{ $supervisor->name }}</p>
        </div>

        <form method="POST" action="{{ route('supervisors.update', $supervisor) }}" class="p-6 space-y-5">
            @csrf @method('PUT')
