...
@section('page-subtitle', 'Perbarui informasi akun operator')
...
<div class="max-w-xl mx-auto">
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
        ...
        <div class="bg-linear-to-r from-amber-500 to-orange-500 px-6 py-5">
            <h2 class="text-lg font-bold text-white">Edit Operator</h2>
            <p class="text-amber-100 text-sm mt-1">{{ $operator->name }}</p>
        </div>

        <form method="POST" action="{{ route('operators.update', $operator) }}" class="p-6 space-y-5">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <select name="department"
                    ...
                    <option value="{{ $dept->name }}" {{ old('department', $operator->department) == $dept->name ? 'selected' : '' }}>{{ $dept->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-400">Operator yang tidak aktif tidak bisa login</p>
                </div>
            </div>
